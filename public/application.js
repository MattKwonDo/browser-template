...
/***/ (function(module, exports, __webpack_require__) {
	...
	var showItemsTemplate = __webpack_require__(14);
	var api = __webpack_require__(35); // has ajax codes that connect to the backend

	// read
	var getItemsSuccess = function getItemsSuccess(data) {
	  // console.log(data)
	  var showItemsHtml = showItemsTemplate({ items: data.new_items });
	  // debugger;
	...
	var createItemsSuccess = function createItemsSuccess(data) {
	  ...
	  $('#create-item').trigger('reset');
	  // $('.content').empty()
	  api.getItems().then(getItemsSuccess).catch(getItemsFailure);
	};

	var createItemsFailure = function createItemsFailure() {
	  // (error) => {
	  // console.error(error)
	...
	var updateItemsFailure = function updateItemsFailure() {} // (error) => {
	// console.error(error)


	// delete
	;var deleteItemsSuccess = function deleteItemsSuccess(id) {
	  // console.log('delete success')
	  // console.log(id)
	  $('#total-' + id).hide();
	};

	var deleteItemsFailure = function deleteItemsFailure() {// (error) => {
	  // console.error(error)
	};
	...
	module.exports = {
	  ...
	  // create
	  createItemsSuccess: createItemsSuccess,
	  createItemsFailure: createItemsFailure,
	  // update
	  updateItemsSuccess: updateItemsSuccess,
	  updateItemsFailure: updateItemsFailure,
	  // delete
	  deleteItemsSuccess: deleteItemsSuccess,
	  deleteItemsFailure: deleteItemsFailure
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "\n<!-- "
	    ...
	    + "\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"container col-md-12 box items "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + " list-unstyled\">\n    <form class=\" "
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + " update-item container\" id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\">\n      <!-- update -->\n        <input type=\"text\" id=\"input-"
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" placeholder=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
	    + "\" required>\n        <input type=\"submit\" value=\"Update Item!\" class=\"update-item\">\n      <!-- remove -->\n        <input type=\"submit\" id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" value=\"Remove Item!\" data-id=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"remove-item\">\n    </form>\n  </li>\n";
	},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
	    var stack1;

	  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : {},(depth0 != null ? depth0.items : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    return "<br>I'm a partial that is included every <big><i>items</i></big> iteration\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var createItem = function createItem(ajaxSend) {
	  return $.ajax({
	    url: config.apiOrigin + '/new_items',
	    method: 'POST',
	    headers: {
	...
	var updateItem = function updateItem(item, ajaxSend) {
	  return $.ajax({
	...
	var deleteItems = function deleteItems(item) {
	  return $.ajax({
	    url: config.apiOrigin + '/new_items/' + item,
	    method: 'DELETE',
	    headers: {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var api = __webpack_require__(35); // has ajax codes that connect to the backend
	var ui = __webpack_require__(13); // has the code for displaying the front end after connecting to the backend
	...
	var onCreateItem = function onCreateItem(event) {
	  event.preventDefault();
	  var nameValue = document.getElementById('new-item-input').value;
	  ...
	  api.createItem(ajaxSend).then(ui.createItemsSuccess).catch(ui.createItemsFailure);
	...
	var onUpdateItem = function onUpdateItem(event) {
	  ...
	  // console.log('id: ' + id)
	  var item = 'input-' + event.target.id;
	  // console.log('item: ' + item)
	  // console.log(this.id)
	  // console.log(this)
	  // console.log(document)
	  var nameValue = document.getElementById(item).value; // this needs to be the id of the input field
	  // console.log('field input :' + nameValue)
	  var ajaxSend = {
	    'new_item': {
	      'name': nameValue // this should be from the form
	  ...
	  api.updateItem(id, ajaxSend).then(ui.updateItemsSuccess).catch(ui.updateItemsFailure);
	};

	// delete
	// if you do this as a fat arrow function, you can't read 'this'
	var onDeleteItems = function onDeleteItems() {
	  event.preventDefault();
	  var id = event.target.id;
	  api.deleteItems(id).then(ui.deleteItemsSuccess(id)).catch(ui.deleteItemsFailure);
	};

	// hide change pw text
	var onClearText = function onClearText() {
	  // clear sign-up errors and text
	  $('.errors-change-password').text('');
	  // $('#change-password').find('input:password, input:password, select, textarea').val('')
	  $('#change-password').trigger('reset');
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "c0de36dba27e5d9e33e466f1fc560337.jpg";
